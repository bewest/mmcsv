...
// "?cpSerialNumber=NONE&msgType=last24hours&requestTime=1444087771183"
module.exports = fetch = (function exec ( ) {
  ...
  function getConnectOpts (daysAgo) {
  ...
  function Fetch (opts) {
    ...
    var is_mm_connect = opts.connect;
    ...
    common.step([
      ...
      function doFetchCSV (response, next) {
        ...
        if (is_mm_connect) {
          var c = getConnectOpts(opts);
          var generates = request.get(config.connect_json, c);
          es.pipeline(generates, zlib.createGunzip( ), out);
        } else {
